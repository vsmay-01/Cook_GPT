...
def delete_file_vectors(user, namespace, filename):
    """Delete vectors associated with a specific file in a namespace."""
    index_name = f"{user}-index"
    try:
        index = pc.Index(index_name)
        
        # Query to find vectors with matching filename
        response = index.query(
            vector=[0] * 768,  # Dummy vector
